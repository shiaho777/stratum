...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v215_gate.bin
...
STRATUM_NOSPEC=1 $BIN "$MODEL" 4 0 1 > /tmp/stratum_v215_bitexact.log 2>&1
python3 - <<'PY'
import re
t=open("/tmp/stratum_v215_bitexact.log").read()
...
echo "== gen8 floor =="
STRATUM_SPINE_DEBUG=1 $BIN "$MODEL" 8 0 1 > /tmp/stratum_v215_gen8.log 2>&1
rg -n "tree-step|generated|WALL|LWC|reseed|path-osf|path-ste|sd-bonus|ofp=|hfw=|shs=|ltp=|xpack=" /tmp/stratum_v215_gen8.log | tee /tmp/stratum_v215_summary.txt
python3 - <<'PY'
import re
t=open("/tmp/stratum_v215_gen8.log").read()
