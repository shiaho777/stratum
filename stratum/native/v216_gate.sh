...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v216_gate.bin
ps=$(pagesize)
...
STRATUM_NOSPEC=1 $BIN "$MODEL" 4 0 1 > /tmp/stratum_v216_bitexact.log 2>&1
python3 - <<'PY'
import re
t=open("/tmp/stratum_v216_bitexact.log").read()
...
PY
echo "== gen8 floor =="
STRATUM_SPINE_DEBUG=1 $BIN "$MODEL" 8 0 1 > /tmp/stratum_v216_gen8.log 2>&1
rg -n "tree-step|generated|WALL|LWC|reseed|path-osf|path-ste|sd-bonus|ofp=|hfw=|shs=|ltp=|xpack=|xsum=" /tmp/stratum_v216_gen8.log | tee /tmp/stratum_v216_summary.txt
python3 - <<'PY'
import re
t=open("/tmp/stratum_v216_gen8.log").read()
...
for k in ("xpack","hfw","shs","ltp","xsum"):
