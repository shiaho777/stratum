...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v204_gate.bin
...
rg -n "tree-step|generated|WALL|LWC|path-gsoed|path-ste|spine-slim|spine-free|sd-bonus|V204 memory" /tmp/stratum_gate_gen8.log | tee /tmp/stratum_gate_summary.txt
python3 - <<'PY'
import re
...
for k in ["path-ste","sd-bonus","spine-slim","path-gsoed","ossg_seeds","path=[198 2 220 16 13 220]"]:
    if k in t: print("marker", k)
lm=re.search(r"V204 LWC:.*?bonus=(\d+) ste=(\d+)/(\d+) ossg=(\d+)/(\d+)", t)
if lm: print(f"bonus={lm.group(1)} ste={lm.group(2)}/{lm.group(3)} ossg={lm.group(4)}/{lm.group(5)}")
if tpm >= 7.99 and mains==1: print("BREAKTHROUGH 8/1")
elif tpm > 4.0 + 1e-9: print("IMPROVED", tpm)
print("GATE PASS")
PY
