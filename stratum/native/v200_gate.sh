...
rg -n "tree-step|generated|WALL|LWC|path-leaf|spine-free|main=" /tmp/stratum_gate_gen8.log | tee /tmp/stratum_gate_summary.txt
python3 - <<'PY'
import re
...
if "path=[198 2 220 16" not in t and "emit=4" not in t and "emit=5" not in t:
    print("WARN: expected emit>=4 path through 16 not found")
print(f"thrash OK tok/main={tpm} mains={int(mains)} tokens={int(n)}")
lm=re.search(r"V200 LWC: ok=(\d+) repair=(\d+) miss=(\d+) pri=(\d+) range=(\d+) tkick=(\d+) join=([0-9.]+)s io_stall=([0-9.]+)s compute=([0-9.]+)s ltfo=(\d+)/(\d+)", t)
if lm:
    print(f"LWC ok={lm.group(1)} repair={lm.group(2)} miss={lm.group(3)} range={lm.group(5)} tkick={lm.group(6)} join={lm.group(7)}s io={lm.group(8)}s ltfo={lm.group(10)}/{lm.group(11)}")
else:
    print("WARN: V200 LWC line missing")
print("GATE PASS")
PY
