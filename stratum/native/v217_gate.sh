...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v217_gate.bin
ps=$(pagesize)
free_pages=$(vm_stat | awk '/Pages free/ {gsub("\\.","",$3); print $3+0}')
free_mb=$(( free_pages * ps / 1024 / 1024 ))
echo "== scene free_MB=$free_mb free_pages=$free_pages =="
echo "== NOSPEC bitexact gen4 =="
STRATUM_NOSPEC=1 $BIN "$MODEL" 4 0 1 > /tmp/stratum_v217_bitexact.log 2>&1
python3 - <<'PY'
import re
t=open("/tmp/stratum_v217_bitexact.log").read()
...
PY
echo "== gen8 floor =="
STRATUM_SPINE_DEBUG=1 $BIN "$MODEL" 8 0 1 > /tmp/stratum_v217_gen8.log 2>&1
rg -n "tree-step|generated|WALL|LWC|path-osf|path-ste|sd-bonus|ofp=|hfw=|shs=|ltp=|xpack=|xsum=|preio=" /tmp/stratum_v217_gen8.log | tee /tmp/stratum_v217_summary.txt
python3 - <<'PY'
import re
t=open("/tmp/stratum_v217_gen8.log").read()
...
for k in ("xpack","hfw","shs","ltp","xsum","preio"):
    m2=re.search(rf"{k}=(\d+)", t)
    if m2: print(f"{k}={m2.group(1)}")
print("GATE PASS")
PY
