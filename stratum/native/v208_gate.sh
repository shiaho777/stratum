...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v208_gate.bin

echo "== NOSPEC bitexact gen4 =="
STRATUM_NOSPEC=1 $BIN "$MODEL" 4 0 1 > /tmp/stratum_v208_bitexact.log 2>&1
python3 - <<'PY'
import re
t=open("/tmp/stratum_v208_bitexact.log").read()
...
echo "== gen8 floor =="
STRATUM_SPINE_DEBUG=1 $BIN "$MODEL" 8 0 1 > /tmp/stratum_v208_gen8.log 2>&1
rg -n "tree-step|generated|WALL|LWC|reseed|path-osf|path-ste|tree-slots|sd-bonus" /tmp/stratum_v208_gen8.log | tee /tmp/stratum_v208_summary.txt
python3 - <<'PY'
import re
t=open("/tmp/stratum_v208_gen8.log").read()
...
rs=re.search(r"TREE B=(\d+)", t)
if rs: print(f"reseed TREE B={rs.group(1)}")
if rs and int(rs.group(1)) < 7:
    print("WARN TREE B < 7 after reseed")
print("GATE PASS")
PY
