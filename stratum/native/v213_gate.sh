...
export STRATUM_PTM_PATH=/tmp/stratum_ptm_v213_gate.bin

echo "== NOSPEC bitexact gen4 =="
STRATUM_NOSPEC=1 $BIN "$MODEL" 4 0 1 > /tmp/stratum_v213_bitexact.log 2>&1
python3 - <<'PY'
import re
t=open("/tmp/stratum_v213_bitexact.log").read()
...
echo "== gen8 floor =="
STRATUM_SPINE_DEBUG=1 $BIN "$MODEL" 8 0 1 > /tmp/stratum_v213_gen8.log 2>&1
rg -n "tree-step|generated|WALL|LWC|reseed|path-osf|path-ste|sd-bonus|ofp=" /tmp/stratum_v213_gen8.log | tee /tmp/stratum_v213_summary.txt
python3 - <<'PY'
import re
t=open("/tmp/stratum_v213_gen8.log").read()
